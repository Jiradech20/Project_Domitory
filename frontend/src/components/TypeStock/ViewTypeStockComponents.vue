<template>
  <div>
    <CRow>
      ...
      <CCol :md="2">
        <CButton
          ...
          @click="$router.push('/RegisTypeStock')"
        >
          New
        </CButton>
      </CCol>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="card mb-4">
          <div class="card-header">ตารางประเภทสต็อก</div>
  ...
  setup() {
    ...
    const fetchStockTypes = async () => {
      try {
        ...
        showToast("Error fetching stock types", "error");
      }
    };
    ...
    const filterItems = () => {
      filteredItems.value = types.value
        .filter((item) => {
          const matchesSearch =
    ...
    onMounted(() => {
      fetchStockTypes(); // Fetch stock types on component mount
    });
