<template>
  <div>
    <CRow>
      ...
      <CCol :md="2">
        <CButton
          ...
          @click="$router.push('/RegisPetitionType')"
        >
          New
        </CButton>
      </CCol>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="card mb-4">
          <div class="card-header">ตารางประเภทคำร้อง</div>
    ...
    >
      ...
      <CModalBody>
        <DeletePetitionTypeComponent
          :selectedPetitionType="selectedPetitionType"
...
import DeletePetitionTypeComponent from "./DeletePetitionTypeComponent.vue";
export default {
  name: "ViewPationTypeComponents",
  components: {
    DeletePetitionTypeComponent,
    },
  setup() {
    const columns = ref([
      { key: "ID", label: "รหัส" },
      { key: "Type", label: "ชื่อประเภทคำร้อง" },
    ...
    const selectedPetitionType = ref({});
    ...
    const fetchPetitionType = async () => {
      try {
        ...
        const response = await axios.get("/api/auth/getPetitionTypeForView", {
          headers: {
        ...
        console.error("เกิดข้อผิดพลาดในการดึงข้อมูลประเภทคำร้อง:", error);
      }
    };

    const filterItems = () => {
      filteredItems.value = petitionType.value.filter((item) => {
        const matchesSearch =
    ...
    const showModalDelete = (item) => {
      selectedPetitionType.value = item;
