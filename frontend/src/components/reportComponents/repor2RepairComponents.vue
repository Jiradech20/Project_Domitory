  ...
  <div class="container mt-4 fluid">
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h5>รายงานข้อมูลการแจ้งซ่อมที่ยังไม่ได้รับการซ่อม</h5>
        <div>
            ...
            <table class="table table-striped table-hover">
              ...
              <tbody>
                ...
                <tr v-for="item in reports" :key="item.mainr_ID">
                  <td>{{ item.mainr_ID }}</td>
                  <td>{{ item.fullname }}</td>
                  <td>{{ item.mainr_ProblemTitle }}</td>
                  <td>{{ item.petitionType }}</td>
                  <td>{{ item.roomNumber }}</td>
                  <td>{{ item.mainr_Date }}</td>
                  <td>{{ item.mainr_SuccessDate || "-" }}</td>
                  <td>
            ...
            <div class="row mb-3">
              <div class="col-md-12">
                <!-- <h6 class="text-center">
                  จำนวนการแจ้งซ่อมที่ยังไม่ได้รับการซ่อมตามสถานะ
                </h6> -->
  ...
  setup() {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get("/api/auth/getReport2", { params });
    ...
    const exportExcel = async () => {
      try {
        ...
        const excelData = reports.value.map((item) => ({
          ...
          หัวข้อปัญหา: item.mainr_ProblemTitle,
          ประเภท: item.petitionType,
          ห้อง: item.roomNumber,
          วันที่แจ้ง: item.mainr_Date,
          วันที่ซ่อมเสร็จ: item.mainr_SuccessDate || "-",
          สถานะ: item.status,
        }));
        ...
        XLSX.utils.book_append_sheet(wb, ws, "รายงานแจ้งซ่อม");

        const fileName = `รายงานแจ้งซ่อม_${dateStr}.xlsx`;
    ...
    const exportPDF = async () => {
      try {
        ...
        if (filters.value.startDate && filters.value.endDate) {
          fileName = `${typeStr}_แจ้งซ่อม_${dateStr}ตั้งแต่${formatDate(
            filters.value.startDate
          )}ถึง${formatDate(filters.value.endDate)}.pdf`;
        } else {
          fileName = `${typeStr}_แจ้งซ่อม_${dateStr}.pdf`;
        }
    ...
    const renderBarChart = (chartData) => {
      ...
      const chartTitle =
        filters.value.startDate && filters.value.endDate
          ? `จำนวนการแจ้งซ่อมที่ยังไม่ได้รับการซ่อม (${filters.value.startDate} ถึง ${filters.value.endDate})`
          : "จำนวนการแจ้งซ่อมที่ยังไม่ได้รับการซ่อมทั้งหมด";
    ...
    const fetchChartData = async () => {
      try {
        ...
        const response = await axios.get("/api/auth/fetchChartData2", {
          params: {
        ...
        if (response.data.success && response.data.data) {
          const chartData = {
            ...
            datasets: [
              {
                label: "จำนวนการแจ้งซ่อมที่ยังไม่ได้รับการซ่อม",
