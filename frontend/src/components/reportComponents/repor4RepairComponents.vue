  ...
  <div class="container mt-4 fluid">
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h5>รายงานการเบิกวัสดุและอุปกรณ์ตามช่วงเวลา</h5>
        <div>
      ...
      <div class="card-body">
            ...
            <table class="table table-striped table-hover">
              ...
              <tbody>
                ...
                <tr v-for="item in reports" :key="item.mainr_ID">
                  <td>{{ item.reqlist_requisition_ID }}</td>
                  <td>{{ item.fullname }}</td>
        ...
        <div v-if="activeTab === 'chart'" class="tab-content mt-3">
          ...
          <div v-else>
            ...
            <div class="row mb-3">
              <div class="col-md-12">
                <h6 class="text-center">จำนวนการเบิกวัสดุและอุปกรณ์ตามสถานะ</h6>
  ...
  setup() {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get("/api/auth/getReport4", { params });
    ...
    const exportExcel = async () => {
      try {
        ...
        const excelData = reports.value.map((item) => ({
          รหัส: item.reqlist_requisition_ID,
          ผู้แจ้ง: item.fullname,
          วัสดุ: item.stockname,
          จำนวน: item.quantity,
          หน่วย: item.unit,
          วันที่แจ้ง: item.requisition_Date,
          สถานะ: item.status,
        }));
        ...
        XLSX.utils.book_append_sheet(wb, ws, "รายงานการเบิกวัสดุและอุปกรณ์");

        const fileName = `รายงานการเบิกวัสดุและอุปกรณ์_${dateStr}.xlsx`;
    ...
    const exportPDF = async () => {
      try {
        ...
        if (filters.value.startDate && filters.value.endDate) {
          fileName = `${typeStr}_การเบิกวัสดุและอุปกรณ์_${dateStr}ตั้งแต่${formatDate(
            filters.value.startDate
          )}ถึง${formatDate(filters.value.endDate)}.pdf`;
        } else {
          fileName = `${typeStr}__การเบิกวัสดุและอุปกรณ์__${dateStr}.pdf`;
        }
    ...
    const renderBarChart = (chartData) => {
      ...
      const chartTitle =
        filters.value.startDate && filters.value.endDate
          ? `จำนวนการเบิกวัสดุและอุปกรณ์ (${filters.value.startDate} ถึง ${filters.value.endDate})`
          : "จำนวนการเบิกวัสดุและอุปกรณ์ทั้งหมด";
    ...
    const fetchChartData = async () => {
      try {
        ...
        const response = await axios.get("/api/auth/fetchChartData4", {
          params: {
        ...
        if (response.data.success && response.data.data) {
          const chartData = {
            ...
            datasets: [
              {
                label: "จำนวนการเบิกวัสดุและอุปกรณ์",
